...
import type { IntelligenceJobPayload } from './intelligence-pipeline-worker-once'
...
async function main() {
  ...
  if (!payload.worldBank && !payload.materializeEconomy && !payload.stubMultilateralCollectors) {
    console.error(
  ...
  console.log(`[intelligence-pipeline-enqueue] created job id=${job.id} kind=${kind}`)
