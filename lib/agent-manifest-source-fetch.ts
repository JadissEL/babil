...
import {
  ...
  recordFetchFailure,
  recordFetchSuccess,
} from './intelligence-pipeline/fetch-circuit-breaker';
...
const COMMITTED_MAP_REL = path.join('data', 'agent-manifest-url-map.committed.json');
...
const DEFAULT_CONCURRENCY = Math.max(
  1,
  Math.floor(Number(process.env.AGENT_MANIFEST_FETCH_CONCURRENCY || 3)),
);
const DEFAULT_REFETCH_TTL_MS = Number(process.env.AGENT_MANIFEST_REFETCH_TTL_MS || 86_400_000);
const FETCH_TIMEOUT_MS = Math.max(
  3000,
  Math.floor(Number(process.env.AGENT_MANIFEST_FETCH_TIMEOUT_MS || 15_000)),
);
const MAX_BODY_READ = Math.max(
  50_000,
  Math.floor(Number(process.env.AGENT_MANIFEST_MAX_BODY_BYTES || 2_000_000)),
);
const EXCERPT_MAX = Math.max(
  500,
  Math.floor(Number(process.env.AGENT_MANIFEST_EXCERPT_CHARS || 12_000)),
);
...
async function resolveMapPath(): Promise<string | null> {
  ...
  const local = path.join(process.cwd(), DEFAULT_MAP_REL);
  try {
    await fs.access(local);
    return local;
  } catch {
    const committed = path.join(process.cwd(), COMMITTED_MAP_REL);
    try {
...
function validateEntryShape(e: unknown): e is ManifestUrlMapEntry {
  ...
  if (tpl) {
    ...
    const bad = tpl
      .match(/\{([^}]+)\}/g)
      ?.filter((x) => !/^\{(country|region|slug|encodedCountry)\}$/.test(x));
    if (bad && bad.length > 0) return false;
  }
  if (o.responseKind !== undefined && o.responseKind !== 'json' && o.responseKind !== 'text')
    return false;
  return true;
}
...
async function fetchOneUrl(
  ...
  try {
    ...
    if (isHostCircuitOpen(host)) return { ok: false, error: 'circuit_breaker_open' };
  } catch {
  ...
  try {
    ...
    if (res.ok) recordFetchSuccess(host);
    else recordFetchFailure(host);
    return {
      ok: res.ok,
...
export async function runManifestUrlFetchBatch(opts: {
  ...
  concurrency?: number;
  refetchTtlMs?: number;
  /** When set, only fetch manifest rows in these category ids (specialized pipeline jobs). */
  categoryIds?: string[];
  ...
  if (!loaded) {
    opts.logVerbose?.('manifest_url_fetch_skip', {
      reason: 'no_map_file',
      path: process.env.AGENT_MANIFEST_MAP_PATH ?? COMMITTED_MAP_REL,
    });
    return {
      mapVersion: null,
      totalFetchable: 0,
  ...
  const joinList = buildJoinList(loaded.file).filter(
    (row) => !categoryFilter || categoryFilter.has(row.categoryId),
  );
  if (joinList.length === 0) {
    ...
    return {
      ...
      totalFetchable: 0,
      newCursor:
        typeof opts.memory.manifestFetchCursor === 'number' ? opts.memory.manifestFetchCursor : 0,
      results: [],
  ...
  const results: ManifestFetchResultItem[] = await runLimitedConcurrency(
    ...
    async (row) => {
      ...
      const ifNone =
        typeof prev.etag === 'string' && prev.etag.trim() ? prev.etag.trim() : undefined;
