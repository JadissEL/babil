      ...
      fallback={
        <div className="mx-auto max-w-6xl space-y-8 pb-24">
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm font-medium text-slate-400">Chargement du moteur…</p>
          </div>
        </div>
      }
    >
...
function RecommendationEnginePageInner() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-8 pb-24">
        ...
        <div className="flex min-w-0 items-center gap-3 sm:gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm font-medium text-slate-400">
              Analyse déterministe, graphiques radar et synthèses — même moteur que les
              recommandations.
            </p>
          </div>
        </div>
      ...
      {focusCountryId != null ? (
        <p
          className="rounded-xl border border-amber-500/35 bg-amber-500/10 px-4 py-2 text-sm font-medium text-amber-100"
          role="status"
        >
          <span className="font-black text-amber-300">Contexte pays.</span> Le classement priorisera{' '}
          ...
          <Link
            ...
            className="font-bold text-amber-200 underline decoration-amber-200/50 underline-offset-2 hover:decoration-amber-100"
          >
            Revoir la fiche
          </Link>
          {' · '}
          <Link
            ...
            className="font-bold text-slate-400 underline decoration-slate-500/50 underline-offset-2 hover:text-amber-200 hover:decoration-amber-200/60"
          >
            Effacer le contexte
          </Link>
        </p>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="space-y-2">
              ...
              <p className="text-[11px] leading-snug text-slate-500">
                Messages de contexte sur la 1ʳᵉ reco.
              </p>
            </div>
            <div className="space-y-2">
          ...
          <div className="mb-6 flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
            ...
            {compareMode ? (
              <p className="max-w-xl text-xs text-slate-500 sm:text-right">
                Cochez des pays dans le classement ; les radars comparés apparaissent sous le
                graphique principal.
              </p>
            ) : null}
          </div>
              ...
              <Card className="min-w-0 border-gray-800 bg-[#111827]">
                <CardContent className="space-y-4 p-4 sm:p-6">
                  ...
                  <p className="text-xs text-slate-500">
                    Les pondérations finales combinent ces axes avec votre profil (revenu, épargne,
                    objectif).
                  </p>
          ...
          {compareRecos.length >= 2 ? (
            <Card className="mt-8 min-w-0 border-gray-800 bg-[#111827]">
              <CardContent className="space-y-4 p-4 sm:p-6">
                ...
                <p className="text-xs text-slate-500">
                  Même échelle que le graphique principal — ouvrez « Définitions des axes » sur le
                  radar du dessus pour le détail.
                </p>
                <div className="grid min-w-0 gap-6 sm:grid-cols-2 xl:grid-cols-3">
                  {compareRecos.map((r) => (
                    <div
                      key={String(r.id)}
                      className="min-w-0 rounded-xl border border-white/10 bg-slate-950/30 p-4"
                    >
