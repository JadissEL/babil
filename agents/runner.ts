...
import { appendOrchestrationCycle, loadRunMemory, saveRunMemory } from '../lib/agent-run-memory';
...
async function loadState() {
  memoryState = await loadAgentStateFromDisk(normalizeDomain, id);
}
...
async function processCountryTask(task: CountryTask) {
  ...
  const snapshotInit: CountrySnapshot = {
    ...
    scalar: {
      ...
      business_visa_score:
        typeof existingRow?.business_visa_score === 'number'
          ? existingRow.business_visa_score
          : 5.5,
  ...
  const orch = await persistOrchestrationAndRunMemory(
    task,
    snapshot,
...
async function runWorkerBatch() {
  ...
  for (const task of runnable) {
    ...
    try {
      ...
      } else if (result.score >= COMPLETENESS_TARGET_SCORE && result.criticalMissing.length === 0) {
        task.status = 'done';
        task.nextRunAt = nextDoneAt;
      } else if (plateau) {
        task.status = 'done';
      ...
      void appendSupervisorMetricEvent({
        ...
        logVerbose('supervisor metrics append failed', {
          error: String((err as Error)?.message || err),
        }),
      );
...
async function main() {
  ...
  const fileOrder = await tryLoadWorldCountryRunOrderFile();
  const sequential = SEQUENTIAL_WORLD_ORDER_ENABLED && !!fileOrder && fileOrder.length > 0;
  console.log(
