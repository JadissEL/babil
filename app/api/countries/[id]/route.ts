...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  return withApiRouteLatency(req, API_ROUTE_LATENCY_KEYS.countriesDetail, async () => {
    ...
    try {
      ...
      if (country) {
        ...
        try {
          const observationConfidenceAggregate = await getObservationConfidenceAggregateForCountry(
            country.id,
          );
    ...
    try {
      ...
      if (country) return jsonWithCacheAndWeakEtag(req, country, COUNTRY_DETAIL_CACHE_CONTROL);
    } catch {
      /* 404 below */
    }
