    ...
    <div className="mx-auto max-w-7xl px-6 py-10 pb-20 sm:px-8">
      <div className="mb-10 flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black tracking-tight text-text md:text-4xl">Business & Investment</h1>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
          {filtered.map((c) => {
            ...
            return (
              <div
                key={c.id}
                className="flex flex-col overflow-hidden rounded-[2rem] border border-line bg-surface shadow-card transition-all duration-500 hover:border-success/30"
              >
                <div className="flex items-start justify-between border-b border-line bg-[#f8f2e8] p-8">
                  <div>
                    <h3 className="mb-1 text-3xl font-black text-text">{c.name}</h3>
                ...
                <div className="grid grid-cols-1 gap-8 p-8 md:grid-cols-2">
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      <div className="rounded-2xl border border-line bg-[#f8f2e8] p-4">
                        <div className="mb-1 text-[10px] font-black uppercase tracking-widest text-muted">
                          Droit d&apos;investir
                        </div>
                        <p className="text-sm font-bold text-text">{biz.rights || 'Information non disponible'}</p>
                      </div>
                      <div className="rounded-2xl border border-line bg-[#f8f2e8] p-4">
                        <div className="mb-1 text-[10px] font-black uppercase tracking-widest text-muted">Setup</div>
                        <p className="text-sm font-bold text-text">{biz.setup || 'Information non disponible'}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-6">
                    <h4 className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-muted">
                      <Store className="h-4 w-4 text-accent" /> Micro-business / food
                    </h4>
                    <div className="space-y-4">
                      <div className="rounded-2xl border border-accent/25 bg-accent-soft p-4">
                        <div className="mb-2 flex items-center justify-between">
                          ...
                          <span className="text-xs font-black text-text">{street.opportunity || 'N/A'}</span>
                        </div>
