...
Fichier repo : `docs/google-stitch/assets/page-16-terra-stitch-reference.png`

**Architecture livrée (Stitch v1 — refactor surgical)** : shell **cream `#FDFBF4`** sur tout `/countries/[id]` ; **toute la logique métier préservée** (state, effects, favoris Clerk, télémétrie history, commentaires, print view, PhD teaser, VisitReasons/TravelerQuotes, DrivingRights, IntelligenceProvenance, CountryDbInsights, BlockFeedback). **Sections redesignées au sommet** (changements visibles dans la capture) : (1) **Header bar** = barre cream-foncé avec accent vertical navy, eyebrow `INTELLIGENCE HUB`, titre serif **« {Pays} – {strapline orientation} »** (orientation dérivée de `studyScore/tourismScore`), bloc droit `VISAFLOW SCORE {finalScore}/100` (couleur amber/emerald via `scoreTone`), `STATUS {Schengen|Non-Schengen}`, bouton imprimer. (2) **Hero row** = drapeau emoji (helper `flagFromCountryName` via région), `h1` serif pays, `MapPin` pour région, bouton heart favori (si Clerk user) à droite. (3) **Réalité terrain** = panneau cream légèrement teinté, citation `morocco_insights.reality` italique serif + **`VERIFIED` stamp** circulaire à droite, puis **4 barres horizontales minces** (`ACCESS`/`ACCEPTANCE`/`FRICTION`/`CONFIDENCE` dérivées des 4 scores existants), puis **3 tuiles** (`POPULATION`/`GDP/CAPITA`/`UNEMPLOYMENT` depuis WB matérialisé). Collapsibles `IntelligenceProvenance` et `CountryDbInsights` conservés en dessous. (4) **Data Quality Signals** = bandeau ambre fin avec icône `AlertTriangle` + label `DATA QUALITY SIGNALS` + texte. (5) **Premium Intelligence card** = block navy `#0D1B3E` (réutilise `DeepReportTeaser`). (6) **Appointment Audit** = simple card 3 colonnes (`PLATFORM`/`DIFFICULTY LEVEL`/`AVG. DELAY`) + paragraphe issues. (7) **Driving & Mobility** = lead card (`IDP Required`) au-dessus du `DrivingRightsIntelSection`. (8) **Sidebar (1/3 sticky)** = bloc carte placeholder (map icon), `Embassy Context` (comportement consulaire + Tourism Visa `STRICT` / Work/Study Permits `STRUCTURED` pills), `Darija tip` cream box, `Processing Metrics` (deux barres rouges : `PROCESSING SPEED → SLOW`, `ADMINISTRATIVE FRICTION → HIGH`), `GoogleAd country_detail_sidebar`. (9) Reste du contenu (commentaires, PhD teaser, VisitReasons preview, TravelerQuotes preview, print view) **intact** sous le pli.

**Métadonnées & impression** : `print:hidden` sur le shell, `print:block` du dump textuel conservé tel quel pour l'impression PDF.
