...
81. **ESLint stricter** (`no-explicit-any`, import order) progressivement. *(Livré partiel : `@typescript-eslint/no-explicit-any`: warn ; **`eslint-plugin-import`** + **`import/order`: warn** + passage **`next lint --fix`** pour normaliser l’ordre des imports sur le dépôt — il reste surtout des warnings **`any`** sur les pages publiques.)*
...
Prioriser **3–5 items** à fort impact / faible risque : par ex. **90–91** (observabilité), réduire les **`any`** sur pages publiques (warnings ESLint restants), **poursuite F.84** (extraire d’autres modules depuis `runner.ts`), extension **Prettier** hors périmètre actuel.
