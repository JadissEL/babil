...
> **Livré (lot B.26–B.38 + C.39–C.42) :** top 3 contributions vs baseline neutre — [`lib/score-driver-explain.ts`](../lib/score-driver-explain.ts) + champs `topDrivers` sur [`POST /api/recommendation`](../app/api/recommendation/route.ts) / [`POST /api/probability`](../app/api/probability/route.ts) et UI `/recommendations`, `/recommendation-engine`, `/probability` ; **B.27** — lexique échelles [`lib/score-scale-lexicon.ts`](../lib/score-scale-lexicon.ts), légendes fiche pays + doc moteur corrigée (Prisma visa **1–10** vs UI **0–100**) ; **B.28** — journal append-only [`_data_changelog`](../lib/full-data-changelog.ts) (admin, agent, pipeline) masqué des API publiques via [`materializePublicFullDataForApi`](../lib/country-full-data-materialize.ts) ; **B.29** — marqueurs fiche pays [`lib/country-intelligence-contract-display-snapshot.ts`](../lib/country-intelligence-contract-display-snapshot.ts) + test ; **B.30** — `defaultsUsed`, libellés « non renseigné » / hints breakdown ([`lib/probability-result-display.ts`](../lib/probability-result-display.ts)) ; **B.31** — agrégat confiance `CountryObservation` ([`lib/country-observation-confidence-aggregate.ts`](../lib/country-observation-confidence-aggregate.ts), [`lib/country-observation-confidence-db.ts`](../lib/country-observation-confidence-db.ts)) + champ `observationConfidenceAggregate` sur [`GET /api/countries/[id]`](../app/api/countries/[id]/route.ts) et fiche pays ; **B.32** — anomalies économie + sauts d’observations ([`lib/intelligence-data-anomalies.ts`](../lib/intelligence-data-anomalies.ts), [`lib/intelligence-data-anomalies-db.ts`](../lib/intelligence-data-anomalies-db.ts)), champ `dataQualityAnomalies` sur la fiche ; **B.33** — stratégie `CountryInsight` vs pipeline — [`docs/country-insight-vs-observations.md`](country-insight-vs-observations.md) + encart UI ; **B.34–B.35** — enums partagés [`lib/user-profile-enums.ts`](../lib/user-profile-enums.ts), validation [`POST /api/user/profile`](../app/api/user/profile/route.ts), alignement reco / bac à sable / proba ; **B.36** — masquage partiel `DelegatedApplicationRequest` ([`lib/delegated-application-payload-utils.ts`](../lib/delegated-application-payload-utils.ts)), liste admin `contactEmailMasked`, détail [`GET /api/admin/delegated-application-requests/[id]`](../app/api/admin/delegated-application-requests/[id]/route.ts) avec `payloadRedactionApplied` par défaut et `?fullPayload=1` pour le JSON complet ; **B.37** — export RGPD [`GET /api/user/data-export`](../app/api/user/data-export/route.ts) + [`lib/user-gdpr-export.ts`](../lib/user-gdpr-export.ts), bouton sur [`/profile`](../app/(dashboard)/profile/page.tsx) ; **B.38** — stratégie i18n [`docs/business-strings-i18n.md`](business-strings-i18n.md), [`lib/i18n/`](../lib/i18n/), pilote libellés `topDrivers` dans `score-driver-explain` ; **C.39** — rétention observations : doc + purge [`scripts/prune-country-observations.ts`](../scripts/prune-country-observations.ts), workflow dry-run [`.github/workflows/country-observation-maintenance.yml`](../.github/workflows/country-observation-maintenance.yml) ; **C.40** — compaction [`scripts/compact-country-observations.ts`](../scripts/compact-country-observations.ts) (`npm run db:compact-observations(:dry)`) ; **C.41** — dashboard admin volumes pipeline : extension [`GET /api/admin/intelligence/summary`](../app/api/admin/intelligence/summary/route.ts) (par source, top 40 pays, 25 derniers runs, `observationsWithoutRun`) + onglet **Intelligence** sur [`/admin`](../app/(dashboard)/admin/page.tsx) ; **C.42** — alertes `EnrichmentRun` : [`lib/enrichment-run-alerts.ts`](../lib/enrichment-run-alerts.ts), champ `runAlerts` sur la même API, bandeau admin, [`scripts/check-enrichment-run-alerts.ts`](../scripts/check-enrichment-run-alerts.ts), workflow [`.github/workflows/enrichment-run-alerts.yml`](../.github/workflows/enrichment-run-alerts.yml), [enrichment-run-alerts.md](enrichment-run-alerts.md).
...
43. **Idempotence** renforcée des collecteurs WB (clé métier claire, reprises).
44. **Sources additionnelles** (OECD, IMF, UN) derrière même abstraction `IntelligenceSource`.
45. **File d’attente** async (queue) si collecte dépasse timeout serverless.
