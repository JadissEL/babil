...
- **Cœur métier** : merge statique + DB [lib/countries-prisma-merge.ts](../lib/countries-prisma-merge.ts), scoring [lib/scoring/](../lib/scoring/), pipeline [lib/intelligence-pipeline/](../lib/intelligence-pipeline/), agents [agents/runner.ts](../agents/runner.ts).
...
> **Livré (lot A.21–A.22) :** lecture sans compte sur [`/recommendations`](../app/(public)/recommendations/page.tsx) et [`/probability`](../app/(public)/probability/page.tsx) avec [`PUBLIC_READ_ONLY_DEMO_PROFILE`](../lib/public-read-only-demo-profile.ts) imposé côté API si non connecté ; [`POST /api/recommendation`](../app/api/recommendation/route.ts) et [`POST /api/probability`](../app/api/probability/route.ts) accessibles sans session pour ce flux ; page design system interne [`/design-system`](../app/(dashboard)/design-system/page.tsx) (auth requise) enrichie d’aperçus produit (`FilterBar`, `CountryCard`, `OfficialSourcesCard`, `ExplorerRegionScoreStrip`, `BlockFeedback`).
