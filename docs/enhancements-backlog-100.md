...
81. **ESLint stricter** (`no-explicit-any`, import order) progressivement. *(Livré partiel : `@typescript-eslint/parser` + plugin, **`@typescript-eslint/no-explicit-any`: warn** ; **`eslint-plugin-import`** + **`import/order`: warn** (résolveur TS/Node) dans [`.eslintrc.json`](../.eslintrc.json) — nombreux warnings attendus tant que les imports ne sont pas normalisés.)*
...
84. **Architecture** : scinder `agents/runner.ts` si > seuil de maintenabilité (modules par étape). *(Livré partiel : [`agents/runner-types.ts`](../agents/runner-types.ts), [`agents/runner-constants.ts`](../agents/runner-constants.ts), [`agents/runner-persistence.ts`](../agents/runner-persistence.ts) ; orchestration dans [`agents/runner.ts`](../agents/runner.ts).)*
...
Prioriser **3–5 items** à fort impact / faible risque : par ex. **90–91** (observabilité Sentry / logs structurés), raffinement **E.68** (domaines multiples), nouveaux **types d’événements** webhook, **poursuite F.84** (extraire d’autres modules depuis `runner.ts`), **import/order** ESLint + extension Prettier.
