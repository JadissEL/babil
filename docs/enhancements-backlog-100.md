...
39. **Politique de rétention** `CountryObservation` (TTL, archive cold storage, agrégation par trimestre). *(Partiellement livré : doc + script — voir [country-observation-retention.md](country-observation-retention.md).)*
40. Job **compaction** : garder dernière observation par `(countryId, fieldPath, source)` + historique optionnel.
