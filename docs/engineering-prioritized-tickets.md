...
| **T8** | Sécurité / supply chain (catalogue E) | **E.66** garde RBAC admin. **E.69** en-têtes + HSTS prod. **E.70/E.73/E.74** notes [catalogue-e-security.md](catalogue-e-security.md). **E.72** rate limit `POST /api/comments`. **E.75** garde scope favoris/historique/export. **E.77** `audit:ci` + Dependabot. **Clerk 6** + `await auth()` — [catalogue-e-security.md](catalogue-e-security.md) | Implémenté (partiel E.67–E.68, E.71, E.76 : backlog) |
