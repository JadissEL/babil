...
| **T7** | Perf API (catalogue D) | **D.54** — pagination curseur sur `GET /api/countries`. **D.55** — `Cache-Control` + SWR liste + fiche. **D.56–D.57** — `unstable_cache` + React `cache()` sur liste fusionnée + `revalidateTag` après PATCH admin pays | Implémenté |
...
### Lot API / perf catalogue D (extrait 53–57)
