...
| **T7** | Perf API (catalogue D) | **D.54–D.60** pagination, cache HTTP, `unstable_cache`, Edge doc, corps max, rate limit. **D.61–D.62** timeout WB + Zod reco/proba — [api-edge-rate-limits.md](api-edge-rate-limits.md) | Implémenté |
...
### Lot API / perf catalogue D (extrait 53–62)
...
- Limites Edge / corps / rate limit / timeout WB / Zod moteurs (D.58–D.62) : [api-edge-rate-limits.md](api-edge-rate-limits.md)
