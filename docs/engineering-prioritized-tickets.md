...
| **T7** | Perf API (catalogue D) | **D.54–D.57** pagination + cache HTTP + `unstable_cache` liste. **D.58–D.60** — middleware Edge documenté ; limite corps POST moteurs ; rate limit reco/proba — [api-edge-rate-limits.md](api-edge-rate-limits.md) | Implémenté |
...
### Lot API / perf catalogue D (extrait 53–60)
...
- Limites Edge / corps / rate limit moteurs (D.58–D.60) : [api-edge-rate-limits.md](api-edge-rate-limits.md)
