...
2. **Transparence scores & profil** : le moteur de probabilité enrichit les `reasons` / `strategy` avec **âge** et **objectif** (`goal_type`) — sans changer les pondérations numériques. **UI** : bandeau « Profil pris en compte » sur `/probability` et `/recommendations`. **Reco** : mêmes textes préfixés sur l’explication du **premier** pays classé ; réponse API avec `countrySignals` + `hasPhdStudies` (encart « Fiche pays » et pastille PhD dans le panneau). **Dashboard `/recommendations`** : radar + barres par pays (sélecteur), lien fiche pays. **Probabilités** : `countrySignals`, breakdown étendu (`appointmentEase`, `riskImmigration`), carte « Meilleur choix » et détail pays.
